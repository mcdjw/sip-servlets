 ...
 *  |                      |--------------------->|
 *  |.............................................|
 *          Repeat once more for full audio
 */
...
public class Unmute extends CallStateHandler {
...
//				msgUtility.send(msg);
				...
				if(count<=2){
					state=1;
					this.processEvent(appSession, msgUtility, request, response, timer);
				}else{
					TalkBACMessage msg = new TalkBACMessage(appSession, "call_unmuted");
					msg.setParameter("origin", origin.getURI().toString());
					msg.setParameter("destination", destination.getURI().toString());
					msg.setStatus(response.getStatus(), response.getReasonPhrase());
					msgUtility.send(msg);	
