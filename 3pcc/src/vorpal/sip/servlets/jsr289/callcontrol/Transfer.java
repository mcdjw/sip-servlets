...
public class Transfer extends CallStateHandler {
	...
	public void processEvent(SipServletRequest request, SipServletResponse response) throws Exception {
		...
		case 1:{
			...
			transferRequest = ThirdPartyCallControlServlet.factory.createRequest(appSession, "INVITE", origin, destination);
		...
		case 3:{
			if(status==200){
				...
				String destinationSessionId = (String) appSession.getAttribute(ThirdPartyCallControlServlet.DESTINATION_SESSION_ID);
		...
		case 7: //BYE
			if(status==200){
				...
				appSession = response.getApplicationSession();
				String originSessionId = (String) response.getApplicationSession().getAttribute(ThirdPartyCallControlServlet.ORIGIN_SESSION_ID);
